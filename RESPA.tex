...
The scheme above will be closely related to the extended system RESPA algorithms introduced in Ref.~\citenum{Martyna_1996} if we concentrate the whole thermostat integration in a single time scale with index $k^\ast$, thus making $\Liu_\mathrm{bath}^k = 0$ for all $k \neq k^\ast$.
...
This is exactly what happens to the side and end schemes of Ref.~\citenum{Zhang_2017}.

It turns out, however, that doing configurational sampling at the end of each step is just a convention.



the side and end schemes of Ref.~\citenum{Zhang_2017} are in fact the same integrator
